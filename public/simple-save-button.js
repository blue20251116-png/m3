(()=>{
  ...
  function readStyle(){
    ...
    const color=val('veTitleColorText','titleColorText','veTitleColor','titleColor');if(color)title.color=color;
    const fontKey=val('veTitleFont','titleFont');if(fontKey)title.fontKey=fontKey;
    const strokeColor=val('veTitleStrokeColorText','titleStrokeColorText','veTitleStrokeColor','titleStrokeColor');if(strokeColor)title.strokeColor=strokeColor;
    const strokeWidth=num('veTitleStrokeWidth','titleStrokeWidth');if(strokeWidth!=null)title.strokeWidth=strokeWidth;
    const shadowColor=val('veTitleShadowColorText','titleShadowColorText','veTitleShadowColor','titleShadowColor');if(shadowColor)title.shadowColor=shadowColor;
    const shadowSize=num('veTitleShadowSize','titleShadowSize');if(shadowSize!=null)title.shadowSize=shadowSize;
    const titleY=num('veTitleY','titleY');if(titleY!=null)title.y=titleY;
    ...
    try{const runs=JSON.parse(localStorage.getItem('m3TitleRuns')||'[]');if(Array.isArray(runs))title.titleRuns=runs}catch{}
    ...
    const ccolor=val('veCaptionColorText','captionColorText','veCaptionColor','captionColor');if(ccolor)caption.color=ccolor;
    const cs=val('veCaptionStrokeColorText','strokeColorText','veCaptionStrokeColor','strokeColor');if(cs)caption.strokeColor=cs;
