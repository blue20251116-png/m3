...
function capture(cmd,args){return new Promise((resolve,reject)=>{const child=spawn(cmd,args,{stdio:['ignore','pipe','pipe']});let out='',err='';child.stdout.on('data',d=>out+=d.toString());child.stderr.on('data',d=>err+=d.toString());child.on('error',reject);child.on('close',code=>code===0?resolve(out.trim()):reject(new Error(`${cmd} failed (${code}) ${err.slice(-1200)}`)))})}
...
async function requireOpenAIKey() { const key = await getSetting('OPENAI_API_KEY'); if (!key) throw new Error('관리자 API 설정에서 OpenAI API Key를 먼저 입력하세요'); return key; }
async function extractAudio(videoPath, outPath) { await run('ffmpeg', ['-y','-i',videoPath,'-vn','-ac','1','-ar','16000','-c:a','pcm_s16le',outPath]); }
async function transcribeDiarized(audioPath, key) {
  const bytes = await readFile(audioPath), form = new FormData();
  form.append('file', new Blob([bytes], { type: 'audio/wav' }), 'audio.wav'); form.append('model', DEFAULT_TRANSCRIBE_MODEL); form.append('response_format', 'diarized_json'); form.append('language', 'zh'); form.append('chunking_strategy', 'auto');
  const r = await fetch('https://api.openai.com/v1/audio/transcriptions',{method:'POST',headers:{authorization:`Bearer ${key}`},body:form}); const j=await r.json();
  if(!r.ok) throw new Error(j?.error?.message||`OpenAI transcription ${r.status}`); if(!Array.isArray(j?.segments)) throw new Error('화자 분리 결과에 segments가 없습니다'); return j;
}
function normalizeTranscript(j) {
  const segments=(j.segments||[]).map((s,i)=>({id:String(s.id||`seg_${String(i+1).padStart(3,'0')}`),speaker:String(s.speaker||'A'),start:Math.max(0,Number(s.start)||0),end:Math.max(0,Number(s.end)||0),original:String(s.text||'').trim()})).filter(s=>s.original&&s.end>s.start);
  const ids=[...new Set(segments.map(s=>s.speaker))], speakers=ids.map((id,i)=>({id,displayName:`화자 ${i+1}`,voice:VOICES[i%VOICES.length],voiceIndex:i%VOICES.length}));
...
async function rewriteKorean({ transcript, humorLevel='variety', key }) {
  ...
  for(const src of transcript.segments)if(!dialogues.some(d=>d.id===src.id))dialogues.push({...src,korean:src.original}); dialogues.sort((a,b)=>a.start-b.start);
  const chars=new Map((parsed.speakers||[]).map(s=>[String(s.id),String(s.character||'')])); return {sceneSummary:String(parsed.sceneSummary||''),viralHook:String(parsed.viralHook||''),speakers:transcript.speakers.map(s=>({...s,character:chars.get(s.id)||''})),dialogues};
}
...
function buildAss(dialogues){const events=dialogues.filter(d=>String(d.korean||'').trim()&&Number(d.end)>Number(d.start)).map(d=>`Dialogue: 0,${assTime(d.start)},${assTime(d.end)},Default,,0,0,0,,${assEsc(d.korean)}`).join('\n');return`[Script Info]\nScriptType: v4.00+\nPlayResX: 1080\nPlayResY: 1920\nScaledBorderAndShadow: yes\n\n[V4+ Styles]\nFormat: Name,Fontname,Fontsize,PrimaryColour,SecondaryColour,OutlineColour,BackColour,Bold,Italic,Underline,StrikeOut,ScaleX,ScaleY,Spacing,Angle,BorderStyle,Outline,Shadow,Alignment,MarginL,MarginR,MarginV,Encoding\nStyle: Default,Noto Sans CJK KR,64,&H00FFFFFF,&H000000FF,&H00000000,&H70000000,-1,0,0,0,100,100,0,0,1,5,2,2,70,70,180,1\n\n[Events]\nFormat: Layer,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text\n${events}\n`;}
async function synthesizeSegment({key,text,voice,outPath}){const r=await fetch('https://api.openai.com/v1/audio/speech',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${key}`},body:JSON.stringify({model:DEFAULT_TTS_MODEL,voice:voice||'marin',input:text,response_format:'wav',instructions:'한국어 쇼츠 더빙처럼 자연스럽고 또렷하게. 대사를 과장하지 말고 상황극 리듬을 살려 말한다.'})});if(!r.ok){let msg=`OpenAI TTS ${r.status}`;try{const j=await r.json();msg=j?.error?.message||msg;}catch{}throw new Error(msg);}await writeFile(outPath,Buffer.from(await r.arrayBuffer()));}
async function audioDuration(file){const out=await capture('ffprobe',['-v','error','-show_entries','format=duration','-of','default=noprint_wrappers=1:nokey=1',file]);return Math.max(0,Number(out)||0);}
function atempoChain(rate){let r=Math.max(0.25,Number(rate)||1),parts=[];while(r>2){parts.push('atempo=2.0');r/=2;}while(r<0.5){parts.push('atempo=0.5');r/=0.5;}parts.push(`atempo=${r.toFixed(5)}`);return parts.join(',');}
export async function renderChinaDubbing({ publicDir, renderDir, uploadId, speakers=[], dialogues=[] }) {
  const key=await requireOpenAIKey(),videoPath=safeUploadPath(publicDir,uploadId),validDialogues=(dialogues||[]).filter(d=>String(d.korean||'').trim()&&Number(d.end)>Number(d.start)).sort((a,b)=>Number(a.start)-Number(b.start)); if(!validDialogues.length)throw new Error('더빙할 한국어 대사가 없습니다');
