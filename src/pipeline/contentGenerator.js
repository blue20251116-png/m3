...
[편집]
- 공백 제외 최소 ${MIN_GENERATED_BODY_CHARS}자, 목표 ${TARGET_BODY_CHARS}. 가능하면 2100자 이상 작성한다.
...
async function draftArticle(ctx,plan){const r=await callOpenAi([{role:'system',content:writerSystemPrompt()},{role:'user',content:`[상품 파악 결과]\n${ctx}\n\n[검색의도 설계]\n${JSON.stringify(plan,null,2)}\n\n실제 네이버 블로그에서 읽기 좋은 완성형 글을 작성해. 목표는 공백 제외 2100~2600자다. 각 핵심 소제목 아래에 단순 스펙 나열이 아니라 구매 판단에 도움이 되는 해석을 2~4문장씩 충분히 작성해. 이모지·번호형 소제목·짧은 줄바꿈·선택적 **강조**·장점/아쉬운점·추천대상·구매체크를 자연스럽게 사용해. 이미지 문맥에는 [IMAGE: ...], 제원이 충분하면 [SPEC_CARD: ...]를 넣어. 확인되지 않은 사실은 만들지 마.`}],.7);const p=parseJsonResponse(r,['titles','body']);if(!Array.isArray(p.titles)||p.titles.length<5)throw new QualityHoldError('[NAVER BLOG][AI FAILED] 제목 5개가 생성되지 않았습니다.');return p;}
async function editorialReview(ctx,plan,draft){const r=await callOpenAi([{role:'system',content:'너는 네이버 블로그 최종 편집자이자 SEO 품질 검수자다. 검색엔진 조작용 키워드 반복은 제거하고 검색의도 충족, 모바일 가독성, 정보 정확성, 자연스러운 강조를 높인다. 광고 카피, AI 상투어, 근거 없는 단정, 허위 체험담을 제거한다. 중요한 규칙: 원문의 유효한 정보를 삭제해 글을 짧게 만들지 않는다. JSON으로만 답한다.'},{role:'user',content:`[상품]\n${ctx}\n[기획]\n${JSON.stringify(plan,null,2)}\n[초안]\n${JSON.stringify(draft,null,2)}\n최종 검수: 목표 공백 제외 1900~2600자이며 절대 ${MIN_GENERATED_BODY_CHARS}자 미만으로 줄이지 마. 제목-본문 일치, 첫 150자 상품/메인키워드 자연 등장, 키워드 억지 반복 금지, 짧은 문단/빈줄, 소제목·이모지 절제, 핵심만 **강조**, 스펙/장점/아쉬움/추천대상/구매체크, 확인되지 않은 단정 금지, [IMAGE: ...] 4~8개, 제원 충분 시 [SPEC_CARD: ...] 1개, AI 상투어 제거. 부족한 분량은 새로운 사실을 만들지 말고 이미 확인된 사실의 의미·선택 기준·활용 관점·비교 관점을 확장해서 채워. {"titles":[5개],"body":"최종본문","changes":["수정요약"]}로 답해.`}],.42);return parseJsonResponse(r,['titles','body']);}
async function expandShortArticle(ctx,plan,current,minChars=MIN_GENERATED_BODY_CHARS){const currentLen=bodyLength(current.body);const need=Math.max(500,minChars-currentLen+450);const r=await callOpenAi([{role:'system',content:'너는 네이버 블로그 원고 확장 편집자다. 기존 글을 삭제하거나 요약하지 않고 정보 밀도와 설명을 보강한다. 새로운 제품 사실, 허위 체험, 확인되지 않은 수치는 절대 만들지 않는다. JSON으로만 답한다.'},{role:'user',content:`[상품]\n${ctx}\n[기획]\n${JSON.stringify(plan,null,2)}\n[현재 원고]\n${JSON.stringify(current,null,2)}\n현재 공백 제외 ${currentLen}자다. 최소 ${minChars}자를 반드시 넘기고 가능하면 1800~2300자로 확장해. 최소 ${need}자 분량의 실질적인 설명을 추가한다고 생각해. 기존 문장과 [IMAGE]/[SPEC_CARD] 마커를 보존하면서 확인된 사실의 의미, 어떤 사용자에게 중요한지, 선택 기준, 활용 상황, 구매 전 확인할 점을 구체화해. 같은 말을 반복해서 글자수만 늘리지 마. {"titles":[기존 의미 유지 5개],"body":"확장된 전체본문"}로 답해.`}],.5);return parseJsonResponse(r,['titles','body']);}
function hasCliche(b){const t=String(b||'');return AI_CLICHE_BLACKLIST.some(p=>t.includes(p));}
async function generateTitlesAndBody(extracted,keywords){const structure=pickStructure(extracted),ctx=buildProductContext(extracted,keywords);console.log('[NAVER BLOG][CONTENT PLAN] 검색의도/SEO 설계 시작');const plan=await analyzeSearchIntent(ctx,structure);console.log(`[NAVER BLOG][CONTENT PLAN] intent="${plan.primaryIntent}" questions=${plan.readerQuestions?.length||0}`);console.log('[NAVER BLOG][DRAFT] 네이버 블로그형 초안 생성 시작');const draft=await draftArticle(ctx,plan);console.log(`[NAVER BLOG][DRAFT] length=${bodyLength(draft.body)}`);console.log('[NAVER BLOG][EDITOR] SEO/강조/이모지/가독성 최종 편집 시작');let final=await editorialReview(ctx,plan,draft),len=bodyLength(final.body);console.log(`[NAVER BLOG][EDITOR] length=${len} cliche=${hasCliche(final.body)?'yes':'no'}`);if(hasCliche(final.body)){console.log('[NAVER BLOG][EDITOR RETRY] AI 상투어 제거 재편집');final=await editorialReview(ctx,plan,final);len=bodyLength(final.body);}for(let attempt=1;len<MIN_GENERATED_BODY_CHARS&&attempt<=2;attempt++){console.log(`[NAVER BLOG][LENGTH REPAIR] ${len}자 → 확장 ${attempt}/2`);final=await expandShortArticle(ctx,plan,final);len=bodyLength(final.body);console.log(`[NAVER BLOG][LENGTH REPAIR] result=${len}자`);}if(len<MIN_GENERATED_BODY_CHARS){console.warn(`[NAVER BLOG][LENGTH SOFT PASS] 확장 후 ${len}자. 길이만을 이유로 발행을 차단하지 않고 후단 QualityGuard에 전달합니다.`);}if(hasCliche(final.body))throw new QualityHoldError('최종 편집 후에도 AI 광고형 상투어가 남아 있습니다.');return{titles:final.titles.slice(0,5),body:final.body.trim(),structure,contentPlan:plan};}
