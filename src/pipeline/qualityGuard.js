...
// 아래 항목은 더 이상 발행 차단 사유가 아니다.
// 원본 근거가 약하면 경고만 남겨 글쓰기 품질 점검에 활용한다.
...
function checkQuality({titles,body,extracted,keywords,affiliateUrl,disclosureText}){
 ...
 // 일반적인 제품 설명은 차단하지 않고 경고만 남긴다.
 const unsupportedClaims=findUnsupportedEvidenceClaims(proseBody,extracted);
 if(unsupportedClaims.length){warnings.push(`원본 근거 약한 제품 설명: ${unsupportedClaims.join(' | ')}`);score-=5;}

 const unverifiedEffect=countMatches(proseBody,UNVERIFIED_EFFECT_PATTERNS);
 if(unverifiedEffect.length){warnings.push(`검증되지 않은 효능 표현: ${unverifiedEffect.join(', ')}`);score-=10;}

 // 없는 숫자/용량/인증은 계속 하드 차단한다.
 const allUnverifiedNumbers=[...new Set([...findUnverifiedNumbers(body,extracted),...findUnverifiedNumberUnitPairs(body,extracted)])];
 if(allUnverifiedNumbers.length){issues.push(`상품정보에 없는 숫자/용량/인증 추정치: ${allUnverifiedNumbers.join(', ')}`);score-=20;}
 ...
 const sentences=splitSentences(proseBody),dups=findRepeatedSentences(sentences);if(dups.length){warnings.push(`동일 문장 반복 ${dups.length}건`);score-=5*dups.length;}
 const starts=findRepeatedSentenceStarts(sentences);if(starts.length){warnings.push(`동일 문장 시작 과다 반복: ${starts.map(([k,c])=>`"${k}"x${c}`).join(', ')}`);score-=5*starts.length;}
 ...
 if(!affiliateUrl||!body.includes(affiliateUrl)){issues.push('본문에 affiliateUrl이 없습니다');score-=20;}
 if(!disclosureText||!body.includes(disclosureText)){issues.push('본문에 제휴 고지 문구가 없습니다');score-=20;}

 const bodyLen=proseBody.replace(/\s/g,'').length;
 if(bodyLen<MIN_BODY_CHARS){issues.push(`본문 길이 부족 (${bodyLen}자, 최소 ${MIN_BODY_CHARS}자)`);score-=20;}
 else if(bodyLen<1400){warnings.push(`본문이 목표 대비 짧음 (${bodyLen}자)`);score-=5;}

 const coreToken=clean(extracted?.productName).split(/\s+/).find(t=>t.length>=2);
 if(coreToken&&!proseBody.includes(coreToken)){issues.push(`본문에 상품명 핵심 토큰("${coreToken}")이 없음`);score-=30;}
